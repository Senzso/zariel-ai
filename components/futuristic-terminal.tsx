...
!socialmedia - View social media management interface
...
export function FuturisticTerminal({ isOpen, onClose, onOpen, shouldSpeak, setShouldSpeak }) {
    ...
    switch (cmd) {
      ...
      case '!bundler':
        if (!isWalletConnected) {
          setOutput(prev => [...prev, 'Please connect your wallet first using !connect'])
          break
        }
        setShowBundler(true)
        break
  ...
  return (
    <AnimatePresence>
      ...
      <PreviewModal isOpen={showSocial} onClose={() => setShowSocial(false)}>
        <SocialMediaPreview />
      </PreviewModal>
